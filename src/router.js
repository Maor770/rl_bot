...
const { normalizePhone, normalizeHolidayName } = require('./utils');
const { sendCurrentVideoPage, sendNavigationButtons, sendVideoByNumber } = require('./modules/videos');
const { sendUpcomingHolidaysMenu, sendAllHolidaysMenu, sendHolidayVideos } = require('./modules/holidays');
const { isMeaningfulHebrewQuery, normalizeHebrewSearch } = require('./utils');
const { detectMenuGuidance, detectNoise, localSmartAnswer, tryOpenAIAnswer } = require('./modules/search');
const { formatSingleVideoAnswer } = require('./modules/videos');
...
async function sendWelcomeAndMainMenu(userId) {
  ...
  ], { action: 'main_menu_buttons_1_fast' });
  ...
  ], { action: 'main_menu_buttons_2_fast' });
}
...
async function routeSelection(userId, id, title) {
  ...
  switch (sid) {
    ...
    case 'game_add_child':          return game.startDailyGameFlow(userId);
    case 'game_settings':           return game.sendGameSettings(userId);
    case 'game_set_reminders':      return game.startReminderTimeSetup(userId);
    ...
    case 'game_turn_off_reminders':
      ...
      return wa.sendTextAndLog(userId, 'הבנתי! עצרתי את התזכורות היומיות. תוכלו להפעיל אותן מחדש דרך תפריט המשחק היומי 🤫', { action: 'opt_out_reminders' });

    default: {
      if (sid.startsWith('holiday:')) {
        return sendHolidayVideos(userId, sid.substring('holiday:'.length));
      }
      if (sid.startsWith('rebbe:')) {
        return sendVideosByRebbe(userId, sid.substring('rebbe:'.length));
      }
      if (sid.startsWith('select_child:')) {
        return game.showChildGameMenu(userId, sid.substring('select_child:'.length));
      }
      if (sid.startsWith('do_mission:')) {
        return game.showActionMission(userId, sid.substring('do_mission:'.length));
      }
      if (sid.startsWith('confirm_mission:')) {
        return game.executeActionMission(userId, sid.substring('confirm_mission:'.length));
      }
...
async function handleFreeText(userId, profileName, messageText) {
  ...
  const aiAnswer = await tryOpenAIAnswer(messageText);
  if (aiAnswer && aiAnswer.mode === 'video' && aiAnswer.video) {
