...
async function startChildRegistration(userId) {
  await wa.sendTextAndLog(userId,
    '➕ *מה השם של הילד או הילדה?*\n(כתבו שם פרטי ושם משפחה)',
    { action: 'game_add_child_prompt' }
  );
  await db.setUserState(userId, { expectedInput: 'REGISTER_CHILD_NAME' });
}
...
async function handleOnboardingFlow(userId, input, state) {
  ...
  if (state.expectedInput === 'REGISTER_CHILD_NAME') {
    ...
    const words = childName.split(/\s+/).filter(w => w.length > 0);
    if (words.length < 2 || childName.length > 40) {
      await wa.sendTextAndLog(userId,
        'אנא כתבו שם פרטי ושם משפחה (לפחות שתי מילים), למשל: *משה לוי*',
        { action: 'game_invalid_name' }
      );
      return;
    }
  ...
  if (state.expectedInput === 'REGISTER_CHILD_BIRTHDAY') {
    ...
    if (!bday) {
      await wa.sendTextAndLog(userId,
        'לא הצלחתי לזהות את התאריך 😅\nאנא כתבו יום וחודש עברי, למשל: *יא ניסן* או *כ תשרי*',
        { action: 'game_invalid_bday' }
      );
      return;
    }
  ...
  if (state.expectedInput === 'EDIT_CHILD_NAME') {
    const newName = String(input || '').trim();
    const words = newName.split(/\s+/).filter(w => w.length > 0);
    if (words.length < 2 || newName.length > 40) {
      await wa.sendTextAndLog(userId, 'אנא כתבו שם פרטי ושם משפחה (לפחות שתי מילים)', { action: 'game_invalid_name' });
      return;
    }
    const childId = state.activeChildId;
    if (!childId) { await db.setUserState(userId, { expectedInput: '' }); return startDailyGameFlow(userId); }
    await db.query('UPDATE children SET child_name = $1 WHERE child_id = $2', [newName, childId]);
    await db.setUserState(userId, { expectedInput: '' });
    await wa.sendButtonsAndLog(userId, `✅ השם עודכן ל-*${newName}* בהצלחה!`, [
      { id: `select_child:${childId}`, title: 'חזרה למשחק 🎲' },
      { id: 'game_settings', title: 'הגדרות ⚙️' },
    ], { action: 'game_edit_name_done', childId });
    return;
  }

  if (state.expectedInput === 'EDIT_CHILD_BIRTHDAY') {
    const bday = parseHebrewBirthday(input);
    if (!bday) {
      await wa.sendTextAndLog(userId, 'לא הצלחתי לזהות את התאריך 😅\nכתבו יום וחודש עברי, למשל: *יא ניסן*', { action: 'game_invalid_bday' });
      return;
    }
    const childId = state.activeChildId;
    if (!childId) { await db.setUserState(userId, { expectedInput: '' }); return startDailyGameFlow(userId); }
    await db.query('UPDATE children SET hebrew_birthday = $1 WHERE child_id = $2', [bday, childId]);
    await db.setUserState(userId, { expectedInput: '' });
    await wa.sendButtonsAndLog(userId, `✅ תאריך הלידה עודכן ל-*${bday}* בהצלחה!`, [
      { id: `select_child:${childId}`, title: 'חזרה למשחק 🎲' },
      { id: 'game_settings', title: 'הגדרות ⚙️' },
    ], { action: 'game_edit_birthday_done', childId });
    return;
  }

  await db.setUserState(userId, { expectedInput: '' });
  const { sendWelcomeAndMainMenu } = require('../router');
  return sendWelcomeAndMainMenu(userId);
}

// ── EDIT PROFILE ──────────────────────────────────────────────────────────────

async function showEditChildMenu(userId) {
  const state = await db.getUserState(userId);
  const childId = state.activeChildId;
...
async function startEditChildName(userId) {
  await db.setUserState(userId, { expectedInput: 'EDIT_CHILD_NAME' });
  await wa.sendTextAndLog(userId, '✏️ *מה השם החדש?*\n(שם פרטי ושם משפחה)', { action: 'game_ask_new_name' });
}

async function startEditChildBirthday(userId) {
  await db.setUserState(userId, { expectedInput: 'EDIT_CHILD_BIRTHDAY' });
  await wa.sendTextAndLog(userId, '🎂 *מה תאריך הלידה העברי הנכון?*\n(למשל: *יא ניסן* או *כ תשרי*)', { action: 'game_ask_new_birthday' });
}
...
async function showDailyTriviaProgram(userId) {
  ...
  if (!triviaList.length) {
    await wa.sendButtonsAndLog(userId, '🌟 כל הכבוד! ענית על כל שאלות הטריוויה!\nחזרו מחר לתוכנית חדשה.', [
      { id: 'game_leaderboard', title: 'הדירוג שלי 🏆' },
      { id: `select_child:${childId}`, title: 'חזרה למשחק 🎲' },
  ...
  await wa.sendButtonsAndLog(userId,
    `🧠 *הטריוויה היומית!*\n\n🎬 מתוך תוכנית מס': *${activeProgNum} - ${activeProgTitle}*\nנשארו לך עוד *${progQs.length}* שאלות פתוחות.\n\nבאיזו דרגת קושי תרצו לשחק?`,
    btns, { action: 'game_trivia_daily_program', childId, progNum: activeProgNum }
  );
}
...
async function evaluateTriviaAnswer(userId, missionId, answerNum) {
  ...
  if (!childId) return startDailyGameFlow(userId);
  if (await db.checkIfCompleted(childId, missionId)) {
    return wa.sendButtonsAndLog(userId, 'כבר ענית על השאלה הזו! 😉', [
...
function buildShareText(userId) {
  ...
  return `רציתי להמליץ לכם על הבוט החדש של *רבי לילדים* 🎉🤗\n\n📽️ בכמה לחיצות תקבלו תוכניות וידאו מרתקות, *ללא עלות!* 💎\n\n🎮 *משחק יומי עם משימות, טריוויה ופרסים שווים!*\n\n${shareLink}`;
}

function getShareLink(userId) {
  return `https://wa.me/${config.BOT_PUBLIC_WHATSAPP_NUMBER}?text=שלום+הגעתי+דרך+${normalizePhone(userId)}`;
}
...
async function processReferral(newUserId, referrerPhone) {
  ...
  await db.addShareTicket(normalizePhone(referrerPhone));
  try {
...
async function executeRaffle(raffleType) {
  ...
  const available = coupons.filter(c =>
    ...
    String(c['Raffle_Type'] || 'Diamonds') === raffleType
  );
  if (!available.length) { console.log('[Game] No available coupons'); return; }
  ...
  if (raffleType === 'Share') {
    ...
    const pool = [];
    for (const u of allUsers) { for (let i = 0; i < Number(u.share_tickets || 0); i++) pool.push(u.phone); }
  ...
  if (winnerPhone) { try { const { sendText } = require('../whatsapp'); await sendText(winnerPhone, winnerMsg); } catch (e) {} }
  ...
  for (const c of children) {
    ...
    try {
      ...
      await sendButtonsAndLog(c.phone,
        ...
        [{ id: 'main_general', title: 'בואו נשחק! 🎲' }, { id: 'nav_home', title: 'תפריט ראשי 🏠' }],
...
async function getAvailableMissions(childId) {
  ...
  return rows.filter(r => {
    ...
    return id && String(r['Active'] || '').toUpperCase() === 'TRUE' && !completed.includes(id);
  }).map(r => ({
...
async function getMissionById(missionId) {
  ...
  if (!row) return null;
  return { missionId, content: String(row['Content'] || ''), reward: Number(row['Reward_Diamonds'] || 10), successMessage: String(row['Success_Message'] || '') };
}
...
async function getAvailableTrivia(childId) {
  ...
  return rows.filter(r => {
    ...
    return num && !completed.includes(`TRIVIA_${num}`);
  }).map(r => ({
